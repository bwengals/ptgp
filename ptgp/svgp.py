...
class SVGP:
    ...
    def predict_white(self, t, sigma=None):
        mu = self.variational_mean
        sqrt = self.variational_root_covariance
        z = self.z
        ...
        Lz_inv_Kzt = pt.linalg.solve_triangular(Lz, Kzt, lower=True)  # Lz⁻¹ Kzt
        Ktz_Lz_invT_sqrt = Lz_inv_Kzt.mT @ sqrt  # Ktz Lz⁻ᵀ sqrt
        mean = mut + Lz_inv_Kzt.mT @ mu  # μt  +  Ktz Lz⁻ᵀ μ

        # Keep goin g below is gpjax

        # Ktt  -  Ktz Kzz⁻¹ Kzt  +  Ktz Lz⁻ᵀ S Lz⁻¹ Kzt  [recall S = sqrt sqrtᵀ]
        covariance = (
            Ktt
            - jnp.matmul(Lzs_inv_Kzt.T, Lz_inv_Kzt)
    ...
    def predict_nonwhite(self, t, sigma=None):
        mu = self.variational_mean
        sqrt = self.variational_root_covariance
    ...
    def fit(self, X_data, y_data, optimizer, params=None, n_steps=100_000, model=None):
        ...
        for step in range(n_steps):
            try:
                ...
                if step % 10 == 0:
                    print(f"Iteration: {step}, Loss: {loss_value:.2f}", end="\r", flush=True)
            
            except KeyboardInterrupt:
                break
            
        print(f"Iteration: {step + 1}, Loss: {loss_value:.2f}, finished.", end="\r", flush=True)
